...
class Filling(GridAgentTask):
    """
    ## Description
        The agent will be presented with an image in which a 
        quarter section has been removed. Then it needs to 
        restore the image by selecting the correct missing 
        piece from a set of distractors in the backpack.
    """
    def __init__(self, 
                 match_pieces: int = 1,
                 **kwargs):
    ...
    def generate_scene_and_goal(self):
        ...
        pieces = RandomChoose(RESOURCE["animal"]["pieces"], num = 1)
        ...
        other_pieces = RandomChoose(RESOURCE["animal"]["all_pieces"], num=4-self.match_pieces, exclude=missing_pieces)
        
        if (4 - self.match_pieces) == 1:
            obj = Obj(other_pieces, "animal")
            bag_pieces.append(obj)
        else:
    ...
    def get_info_img(self):
        ...
        target_img = ImgOverlay(ImgZoom(LoadImage("./imgs/game/bag_grid.png"), 1.1), ImgZoom(self.target, 0.9), middle_center=True)
